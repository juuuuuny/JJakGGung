...
function drawRawListData(list) {
	...
	document.getElementById("femaleSize").innerHTML = "<br><br>&nbsp;* 여자 " + femaleList.length + "명";
	document.getElementById("maleSize").innerHTML = "<br> &nbsp;* 남자 " + maleList.length + "명";
...
function match() {
	...
	if (sizeFlag > 0) {
		for (var f=0; f<=maleList.length; f++) {
			...
			AppData.matchedList.push(selectedMale1 + "ㅠㅠ " + (selectedMale2 ? selectedMale2 : ""));
		}
	} else if (sizeFlag < 0) {
		for (var m=0; m<=femaleList.length; m++) {
			...
			AppData.matchedList.push(selectedFemale1 + "ㅠㅠ " + (selectedFemale2 ? selectedFemale2 : ""));
		}
	}
